...
TCPServer::TCPServer(Param param)
{
	parameter = param;
}
...
{
	listen(serverSocket, 30);	//요청 대기할 수 있는 클라이언트 수 설정
...
unsigned __stdcall TCPServer::ThreadProcess(void * param)
{
	...
	RecvFuncType recvFunc = p.recvFunc;
	...
	while (1)
	{
		GetQueuedCompletionStatus(hComPort, &bytesTrans,
			(LPDWORD)&socketInfo, (LPOVERLAPPED*)&ioData, INFINITE);
