...
{
	cout << "GameManager Initializer" << endl;

	Param p;

	p.recvFunc = GameProcess;
	p.socketClosetCallBack = CloseSocketCallBack;
	gameServer = TCPServer(p);
	gameServer.run();
...
void GameManager::GameProcess(LPSOCKET_INFO socketInfo, char* message)
{
	...
	case CHAT:
		...
		GameRoomRepository::BroadCastMessage(gameRoom.userList, (char*)&recvMessage, BUF_SIZE);
		break;
	case READY:
